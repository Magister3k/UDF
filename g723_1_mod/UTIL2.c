...
**  Function:        DotProd()
**
**  Description:     Dot product with SIMD optimization
**
**  Arguments:
**
**  FLOAT *in1
...
FLOAT DotProd(FLOAT *in1, FLOAT *in2, int len)
{
    ...
    /* Horizontal add */
    __m128d vsum_shuf = _mm_shuffle_pd(vsum, vsum, 1);
    vsum = _mm_add_pd(vsum, vsum_shuf);
    double sum2[2];
    _mm_storeu_pd(sum2, vsum);
    sum = (FLOAT)sum2[0];
