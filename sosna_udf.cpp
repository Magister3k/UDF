#include <windows.h>
#include <cstring>
#include <vector>
#include <new>
#include <algorithm>
#include <cstdio>
#include <climits>

#include "g723_1_decoder.h"
#include "g711u_coder.h"

#define G723_SAMPLES_PER_FRAME 240
#define G723_FRAME_SIZE_SID 4
#define G723_MAX_FRAME_SIZE 24
#define G723_MIN_FRAME_SIZE 1

#define INPUT_SEGMENT_SIZE 4096
#define INPUT_BUFFER_SIZE (INPUT_SEGMENT_SIZE + G723_MAX_FRAME_SIZE)

#if defined(_MSC_VER)
#define UDF_ALIGN16 __declspec(align(16))
#else
#define UDF_ALIGN16 __attribute__((aligned(16)))
#endif

// ��������� BLOB-контроля InterBase 2009 (Win32 UDF ABI).
typedef struct blob_callback {
    short   (__cdecl *blob_get_segment) (void*, char*, unsigned short, unsigned short*);
    void*   blob_handle;
    long    blob_number_segments;
    long    blob_max_segment;
    long    blob_total_length;
    void    (__cdecl *blob_put_segment) (void*, char*, unsigned short);
} *BLOB_CB;

static bool g_decoder_initialized = false;
static CRITICAL_SECTION g_decoder_lock;

class DecoderLockGuard {
public:
    explicit DecoderLockGuard(CRITICAL_SECTION* lock) : lock_(lock) {
        EnterCriticalSection(lock_);
    }

    ~DecoderLockGuard() {
        LeaveCriticalSection(lock_);
    }

    DecoderLockGuard(const DecoderLockGuard&) = delete;
    DecoderLockGuard& operator=(const DecoderLockGuard&) = delete;

private:
    CRITICAL_SECTION* lock_;
};

// ��������� ����������� ����������� ������ ��� ������ �� SSE/AVX Alignment Fault
UDF_ALIGN16 static unsigned char g_output_buffer[SHRT_MAX];
UDF_ALIGN16 static double g_pcm_output[G723_SAMPLES_PER_FRAME];

BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    (void)hModule; (void)lpReserved;

    if (ul_reason_for_call == DLL_PROCESS_ATTACH) {
        OutputDebugStringA("[UDF] DllMain: DLL_PROCESS_ATTACH\n");
        InitializeCriticalSection(&g_decoder_lock);
        g711u_init_encoder();
        g_decoder_initialized = true;
    }
    else if (ul_reason_for_call == DLL_PROCESS_DETACH) {
        OutputDebugStringA("[UDF] DllMain: DLL_PROCESS_DETACH\n");
        g_decoder_initialized = false;
        DeleteCriticalSection(&g_decoder_lock);
    }
    return TRUE;
}

static inline void encode_pcm_to_pcmu(const double* pcm_input, unsigned char* output, int sample_count) {
    for (int i = 0; i < sample_count; ++i) {
        short sample_short = static_cast<short>(pcm_input[i]);
        output[i] = g711u_linear_to_pcmu(sample_short);
    }
}

static inline void flush_output_buffer(BLOB_CB out_blob, unsigned char* output_buffer, size_t& output_idx) {
    if (output_idx > 0) {
        out_blob->blob_put_segment(out_blob->blob_handle,
                       reinterpret_cast<char*>(output_buffer),
                       static_cast<unsigned short>(output_idx));
        output_idx = 0;
    }
}

static void transcode_internal(BLOB_CB in_blob, BLOB_CB out_blob) {
    OutputDebugStringA("[UDF] ������ transcode_internal - ����� ���������\n");

    if (!in_blob->blob_get_segment || !out_blob->blob_put_segment) {
        OutputDebugStringA("[UDF] ������: �������� BLOB callback\n");
        return;
    }

    size_t output_idx = 0;
    size_t pending_size = 0;
    bool received_input = false;
    UDF_ALIGN16 unsigned char input_buffer[INPUT_BUFFER_SIZE] = {};

    // ������� �������� ��������
    G723DecoderContext* decoder_ctx = g723_create_context();
    if (!decoder_ctx) {
        OutputDebugStringA("[UDF] ����������� ������: �� ������� ������� �������� �������� g723\n");
        return;
    }

    unsigned short bytes_read = 0;
    short result = 0;

    if (in_blob->blob_total_length < 0) {
        OutputDebugStringA("[UDF] ������: ������������ ������ BLOB\n");
        g723_destroy_context(decoder_ctx);
        return;
    }

    bool end_of_blob = false;
    while (!end_of_blob) {
        bytes_read = 0;
        result = in_blob->blob_get_segment(
            in_blob->blob_handle,
            reinterpret_cast<char*>(input_buffer + pending_size),
            static_cast<unsigned short>(INPUT_SEGMENT_SIZE),
            &bytes_read
        );

        if (bytes_read > INPUT_SEGMENT_SIZE ||
            pending_size + bytes_read > sizeof(input_buffer)) {
            OutputDebugStringA("[UDF] ������: �������� blob_get_segment\n");
            break;
        }

        pending_size += bytes_read;
        received_input = received_input || bytes_read != 0;
        end_of_blob = result != 0;

        size_t input_pos = 0;
        while (pending_size - input_pos >= G723_FRAME_SIZE_SID) {
            const unsigned char first_byte = input_buffer[input_pos];
            int current_frame_size = 0;

            switch (first_byte & 0x03) {
                case 0x00: current_frame_size = G723_MAX_FRAME_SIZE; break;
                case 0x01: current_frame_size = 20; break;
                case 0x02: current_frame_size = G723_FRAME_SIZE_SID; break;
                default: current_frame_size = 1; break;
            }

            if (pending_size - input_pos < static_cast<size_t>(current_frame_size)) {
                break;
            }

            int consumed = g723_decode_frame(
                decoder_ctx, &input_buffer[input_pos], g_pcm_output);
            if (consumed <= 0) {
                ++input_pos;
                continue;
            }

            input_pos += static_cast<size_t>(consumed);
            if (output_idx + G723_SAMPLES_PER_FRAME > sizeof(g_output_buffer)) {
                flush_output_buffer(out_blob, g_output_buffer, output_idx);
            }

            encode_pcm_to_pcmu(
                g_pcm_output, &g_output_buffer[output_idx], G723_SAMPLES_PER_FRAME);
            output_idx += G723_SAMPLES_PER_FRAME;

            if (output_idx >= static_cast<size_t>(SHRT_MAX - G723_SAMPLES_PER_FRAME)) {
                flush_output_buffer(out_blob, g_output_buffer, output_idx);
            }
        }

        if (input_pos > 0) {
            pending_size -= input_pos;
            std::memmove(input_buffer, input_buffer + input_pos, pending_size);
        }

        if (!end_of_blob && bytes_read == 0) {
            OutputDebugStringA("[UDF] ������: BLOB �� �������� ������\n");
            break;
        }
    }

    if (!received_input) {
        OutputDebugStringA("[UDF] ��������: ������� BLOB ����, ���������������� ��������\n");
        g723_destroy_context(decoder_ctx);
        return;
    }

    flush_output_buffer(out_blob, g_output_buffer, output_idx);

    g723_destroy_context(decoder_ctx);
}

extern "C" void __stdcall transcode_g723(BLOB_CB in_blob, BLOB_CB out_blob) {
    if (!g_decoder_initialized || !in_blob || !out_blob) return;
    DecoderLockGuard lock_guard(&g_decoder_lock);
    try {
        transcode_internal(in_blob, out_blob);
    }
    catch (...) {
        OutputDebugStringA("[UDF] ������: ���������� ���������� ����������\n");
    }
}

extern "C" void __stdcall transcode_g723_ib_util(BLOB_CB in_blob, BLOB_CB out_blob, void* (*ib_util_malloc)(size_t), void (*ib_util_free)(void*)) {
    (void)ib_util_malloc; (void)ib_util_free;
    if (!g_decoder_initialized || !in_blob || !out_blob) return;
    DecoderLockGuard lock_guard(&g_decoder_lock);
    try {
        transcode_internal(in_blob, out_blob);
    }
    catch (...) {
        OutputDebugStringA("[UDF] ������: ���������� ���������� ����������\n");
    }
}
